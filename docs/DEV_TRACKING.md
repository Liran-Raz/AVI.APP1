...
| DEV-029 | ביקורת-אבטחה אדוורסרית 2 + הקשחת גבול-הכתיבה ב-DB | אבטחה (רב-סבב) | P1 | **R1+R2+R3 חיים בפרודקשן (main `1097aa6`) · נותר: החלת-0030 ע"י מפעיל → 9/9** | 2026-07-21 | נובע מחשש שהעלה Liran ("Supabase מול Google — מי בטוח יותר?"). **מועצת-LLM = להישאר על Supabase** (פריצות הן ברמת-אפליקציה, לא ספק-תשתית; Firebase מוסיף מודל-בידוד שני + SDK-דפדפן = יותר משטח-תקיפה; המיגרציה עצמה היא הסיכון). **ביקורת red-team (4 סוכנים, ברמת-קוד):** תכשיט-הכתר — **בידוד בין-דיירים 🟢 איתן (0 דליפה)**; 9 ממצאים, **אף אחד לא בין-דייר**: 1 גבוה (#1 מנהל→בעלים דרך כתיבה-ישירה ל-PostgREST; שורש = הגבלות-תפקיד רק בשכבת-השירות בעוד הטבלאות פתוחות לכתיבה ל-`authenticated`) + בינוניים/נמוכים. **🟢 R2 (שכבת-אפליקציה, [PR #104](https://github.com/Liran-Raz/AVI.APP1/pull/104), main `9745aa3`, חי):** #3 open-redirect ב-/login (`sanitizeNextPath` + הקשחתה לחסום גם `\`, חולץ ל-`lib/safe-path.ts`) + #6 הזרקת-נוסחאות-CSV (`toCsv` מקדים `'` לתאי-טקסט, עם שמירה על סכומים-שליליים). **🟢🟢 R1 (הקשחת-DB, מיגרציה `0029`, [PR #105](https://github.com/Liran-Raz/AVI.APP1/pull/105), main `a342083`; הוחלה+אומתה בפרודקשן ע"י Liran):** טריגרי-שמירה `SECURITY INVOKER` עם שער `current_user='postgres'` המשקפים את `team.service` 1:1 (**אפס שינוי-קוד**) + `revoke insert/delete` על חברויות + `delete` על clients + פיצול-מדיניות client_contacts + 12 החלפות עוזר-מיושן; postflight 3/4/0; **+ job-CI קבוע `validate-write-hardening`** שמוכיח התנהגותית על Postgres אמיתי שכל תקיפה נחסמת וכתיבות-לגיטימיות עוברות. **🟢 R3 מוזג+חי 2026-07-21 ([PR #106](https://github.com/Liran-Raz/AVI.APP1/pull/106) → main `1097aa6`, smoke ירוק כולל הוכחות-חיות):** #8 fail-closed בפרודקשן · #9 CSP נאכף (פרגמטי; nonce=DEV-030) · .strict() על 46 סכמות · מיגרציה `0030` org-pin (ממתינה להחלת-מפעיל — חבילה בגוף-ה-PR; האחסון→`0031`). ראה פירוט בסעיף DEV-029. |
...
**🟢 R3 — תפעול + מידע (מוזג+חי: [PR #106](https://github.com/Liran-Raz/AVI.APP1/pull/106) → main `1097aa6` ‏2026-07-21, במילת-האישור של Liran "תבדוק ותמזג"; פריסת-פרודקשן אומתה חיה — כותרת-ה-CSP הנאכפת מוגשת בווריאנט-הפרודקשן [בלי unsafe-eval/vercel.live], מפתח-זר-ב-body ⇒ ‏400 ‏"Unrecognized key", ‏signin ⇒ ‏401 נקי [לא 503 ⇒ ‏Upstash מוגדר ופעיל]; **נותר: החלת `0030` ע"י המפעיל**):**
...
**סטטוס:** ‏**9/9 ממצאים סגורים+חיים בפרודקשן** (R1: ‏#1/#2/#4/#5/#7 · R2: ‏#3/#6 · R3: ‏#8/#9 +
‏.strict()) — **נותרה פעולה תפעולית אחת: החלת `0030` (org-pin) ב-Dashboard ע"י Liran**, ואז גם
פריט-המידע האחרון סגור והביקורת מוצתה. המשך-עתידי
