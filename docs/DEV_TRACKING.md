...
| DEV-010 | בינאום מלא (i18n) — 8 שפות | UX/i18n | P2 | **🟢 סבב 1 (he+en) הושלם — כל האפליקציה דו-לשונית** | 2026-07-18 | **סבב-1 (עברית+אנגלית) הושלם וחי בפרודקשן** (main `ba207477`, קטלוג 893 מפתחות). כל מסכי האפליקציה תורגמו he/en עם מראה-LTR נכון: הגדרות/משימות/לקוחות/צוות+תפקידים/דשבורד/הודעות/הנהלת-חשבונות/דוחות/לוח/onboarding+הזמנות/שלד/שאריות-chrome (שעון+חיווי topbar, מתג-משרדים, אכיפת-2FA, דיווח-באג) **+ עמודי ה-auth** (login/signup/mfa/שכחתי/איפוס — אוחדו לקטלוג המרכזי, PR-12). PRs #85–[#96](https://github.com/Liran-Raz/AVI.APP1/pull/96). אנגלית זמינה, עברית ברירת-מחדל (משתמשים קיימים ללא שינוי; עברית זהה-בייט). כל PR: tsc/lint נקי, 560 בדיקות, ללא מיגרציה/deps, Vercel ירוק, QA+מיזוג באישור Liran. סריקת-grep סופית נקייה (עברית שנותרה = מפורש: PDF-מס, privacy/terms, שיווק; + הערות). guard-rail: PDF/CSV/מיילים/התראות-DB נשארים עברית. **סבב-2 מוקפא בהחלטת Liran 2026-07-18 — נשארים על עברית+אנגלית בינתיים.** כשיוחלט: סבב-2 = 6 השפות (ru/de/fr/ja/it/ar) — תרגום JSON (המפתחות קיימים) + פונטים + RTL לערבית. ראה memory `feature_dev010_i18n.md`. |
...
| DEV-013 | אימות דו-שלבי (2FA) | אבטחה | P2 | **🟢 חי בייצור** | 2026-07-17 | **מוזג — [PR #84](https://github.com/Liran-Raz/AVI.APP1/pull/84) → main `3d70d1c`.** TOTP (אפליקציית-אימות) דרך Supabase MFA; אופציונלי לכל משתמש + **חובה משרדית קשיחה** (בעלים מחייב → מי שלא הגדיר מגיע רק להגדרות). דיאלוג-השבתה נסגר-אוטומטית אחרי 15 שנ'. **חי אך אינרטי** (בלי דגל, אבל opt-in: require_mfa=false כברירת-מחדל, אף אחד לא רשום). כל השערים בוצעו: TOTP הופעל ב-Supabase, מיגרציה `0028` הוחלה+אומתה, QA של ליראן עבר. תיקן באג-רדום: `<Toaster/>` לא היה מעוגן. נוהל שחזור: `docs/2FA_RECOVERY.md`. 556 בדיקות. ראה פירוט. |
...
| DEV-026 | מודול הנהלת-חשבונות (חשבוניות/קבלות) + דוחות + **רישום תוכנה ברשות המסים** | פיתוח (רב-סבב) | P2 | **R1–R4 פרוסים בפרודקשן (רדומים, דגל כבוי) · הסימולטור עבר (ליקוי 1006 בלבד) · R5 נפתח — ממתין לאישור sandbox** | 2026-07-17 | סבב-על **ledger-first** (שלב א': המשרד מפיק לעצמו; שלב ב' עתידי: ספרים ללקוחות). v1: מסמכים **305/320/330/400** · מסמכים ממוחשבים (R6) · חשבוניות-ישראל (R5; סף ₪10K→₪5K ב-2026) · **מבנה אחיד v1.31** (R4) · רישום במערכת המקוונת (R7). **R1** = מיגרציה **`0027`** (ledgers+documents+counters ללא-דילוגים+קפיאון+RPCs, fail-closed) + פרופיל-עסק + הרשאות `invoices.*` (post-0012) + דגל `INVOICING_UI` (כבוי). אל-יעדים: אין הנה"ח-כפולה/מלאי (INI 1013=0). **בולע את DEV-012** (לוגו+ח.פ על ה-ledger). שערי-בעלים: פרטי-עסק, שער-ממשלתי, תעודת-CA, סימולטור, הגשה. ראה פירוט. |
...
**R5 (בתהליך — נפתח 2026-07-17):** חשבוניות-ישראל (מספרי-הקצאה) —
- **הוראות רישום-מפתחים מלאות נמסרו לליראן** (מקורות רשמיים: מדריך-ההרשמה OpenApiUserGuide של
  שע"ם [נקרא במלואו] + מצגת תהליך-העבודה + תיאור-ה-API v2.0/7.2024). הזרימה: משתמש אישי בשירותים
  הדיגיטליים (בלי כרטיס חכם) → **רישום ל-sandbox קודם — תנאי-מקדים מחייב**
  (openapi-portal.taxes.gov.il/sandbox) → ארגון בפורטל-הייצור + מסמכים חתומים
  (gov.il/he/service/connect-to-shaam) → מייל-אישור → Create App (CLIENT ID+SECRET — ה-SECRET
  מוצג פעם אחת בלבד) → Subscribe למוצר **Invoices**.
- **ליראן נרשם ל-sandbox ביום ו' 2026-07-17; החשבון במצב "pending approval".** שגיאת
  "Multiple sign-ups" בכניסה חוזרת = התנהגות-ההמתנה המתועדת (עמ' 5 במדריך). **צפי מייל-אישור:
  ראשון–שני 19–20.7** (1–2 ימי-עסקים מעל הסופ"ש; לבדוק גם ספאם; התעכב —
  ITAOpenApiSupport@taxes.gov.il).
- **פעולה מקבילה שאינה חסומה:** הכנה + הגשה של מסמכי-הארגון החתומים לפורטל-הייצור (המסלול
  הארוך). צד-פיתוח: רוב R5 (מכונת-מצבים, ספים effective-dated, לקוח-OAuth, UI) ניתן לבנייה על
  mocks עוד לפני credentials — לפי החלטת ליראן.
- עובדות טכניות שאומתו: OAuth2 authorization-code; טוקן 100 שעות + refresh; בסיס-API
  ita-api.taxes.gov.il/shaam/{tsandbox|production}/Invoices/v2 (Approval/MultApproval +
  invoice-information + InvoiceDecision); משתמשי-דמו ל-sandbox; דוגמאות-קוד
  github.com/isrtaxesopenapi; ספים: 2025=₪20K → 1.1.2026=₪10K → 1.6.2026=₪5K.
