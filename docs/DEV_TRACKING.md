...
| DEV-027 | נגישות אתרים (ת״י 5568) — הצהרת נגישות + תיקוני-נגישות | נגישות/משפטי | P2 | **הצהרה + כל תיקוני-הקוד (סבבים A/B/C/D) חיים בפרודקשן** | 2026-07-18 | נובע מחוות-דעת שביקש Liran על סקיל-נגישות צד-שלישי (agentskills.co.il/skills-il) — **הסקיל נבדק ונמצא לא-זדוני** (סקריפט audit_a11y.py סטטי-תמים, ללא exfiltration/injection; יש סקיל Anthropic רשמי מקביל). ביקורת עמוד-הבית מצאה **בסיס טוב** (landmarks/h1/מקלדת/focus/skip-link/prefers-reduced-motion/labels/RTL) אבל **פער משפטי גדול: לא הייתה הצהרת נגישות** (חובה לפי תקנות התאמות-נגישות-לשירות 2013 + ת״י 5568/WCAG 2.0 AA). **שלב 1 שוגר** ([PR #97](https://github.com/Liran-Raz/AVI.APP1/pull/97), main `32fb2e8`): עמוד `/accessibility` (עברית מחייבת) + `/accessibility/en` בתבנית privacy/terms + קישור-פוטר; רכז=לירן רז/liran995@gmail.com/050-8880981; מפעיל=עוסק מורשה 314954835. ללא מיגרציה/deps, 560 בדיקות. **🔴 תיקון-נוסח משפטי ([PR #99](https://github.com/Liran-Raz/AVI.APP1/pull/99), main `e8720df`):** הנוסח הראשון פירט **רשימת-כשלים ספציפית** (חשיפה משפטית — הודאה באי-תאימות). שוכתב לתבנית הישראלית התקנית (מודל rail.co.il ש-Liran סיפק): הצהרה חיובית + בסיס-חוקי (חוק 1998) + רשימת-התאמות עשירה (מקלדת/רספונסיבי/הווידג'ט) + **הסתייגות כללית** ("ייתכן שחלקים טרם הונגשו... נתקן בהקדם") — **בלי רשימת-כשלים ובלי "תואם-100%" שקרי**. לא הומצאו טענות-בדיקה שאין לנו (ביקורת-חברה/מטריצת-מכשירים כמו הרכבת). **מומלץ מבט יועץ-משפטי (לא-חוסם).** **🟢 תיקוני-הקוד שוגרו 2026-07-19 (סבבים A–D, PR #100–#103 — פירוט מלא בסעיף DEV-027 למטה). הפערים שהיו מפורטים כאן — נסגרו:** 🟠 טפסים בלי `autocomplete` + שגיאות-toast לא-מוצמדות → **סבבים A+D** (רכיב `FormError` `role=alert` + `aria-invalid`/`aria-describedby` + `autoComplete`, בכל טפסי-ה-auth הציבוריים ובכל 8 הטפסים הפנימיים) · זוגות-ניגודיות גבוליים (`#67718a`) → **סבב C** (`--ink-faint`→`#5e6880`; לבן-על-`#2563eb`=5.17 עובר, לא נגעו) · מוקאפים-מזויפים בנחיתה → **סבב B** (`aria-hidden`) · אין skip-link/`<main>` בעמודי-auth → **סבב A** (landmark + skip-link בכל 6). **נותרו מינוריים בלבד:** 🟡 הסתרת-SVG דקורטיבי עקבית · `lang` פר-קטע · (אופציונלי: ביקורת-נגישות מקצועית). חוות-הדעת המלאה: plan `https-agentskills-co-il-...`. |

| DEV-028 | ווידג'ט נגישות (לחצן צף + תפריט התאמות) | נגישות/UX | P2 | **חי בפרודקשן** | 2026-07-18 | בהמשך ל-DEV-027 — Liran ביקש לחצן-נגישות בסגנון אתר-הסקילים הישראלי. **ווידג'ט עצמאי שמחיל התאמות-CSS אמיתיות** (לא אוברליי-צד-שלישי). [PR #98](https://github.com/Liran-Raz/AVI.APP1/pull/98) → main `92d5cdb`. **9 התאמות** (attribute על `<html>` → globals.css): גודל-טקסט (3 רמות)·ניגודיות (override טוקנים `:root`+`.mkt`)·הדגש-קישורים/כותרות·גופן-קריא·מרווח-שורות·עצור-אנימציות (site-wide)·סמן-גדול/שחור. נשמר ב-localStorage; `beforeInteractive` מ-`/public/a11y-init.js` מחיל לפני-צביעה (no-flash). Radix Dialog (focus-trap/restore/aria-modal/Escape); נגיש-בעצמו; דו-לשוני. **מיקום (בקשת Liran):** הכפתור הצף **רק בעמודים ציבוריים** (נחיתה/auth/onboarding); בתוך האפליקציה — **כרטיסיית "נגישות" ב-`/settings`** (אין FAB). קוד משותף: `useA11yPrefs` hook + `AccessibilityControls` (tone dark/light) בין הפאנל-הצף לכרטיסייה. מוקאפ אושר קודם (נייבי, ימין-תחתון). ללא מיגרציה/deps. 4 באגים נתפסו בבדיקה (stale-closure·unmount-hang מ-exit-animation×Presence×עצור-אנימציות·focus-restore·אזהרת script-tag של React→קובץ סטטי). guard-rail: התאמות-CSS בלבד, לא תחליף לתיקוני-הנגישות ב-DEV-027 (**שוגרו 2026-07-19 — סבבים A/B/C/D**). |

| DEV-029 | ביקורת-אבטחה אדוורסרית 2 + הקשחת גבול-הכתיבה ב-DB | אבטחה (רב-סבב) | P1 | **הושלם — הביקורת מוצתה: 9/9 + פריטי-המידע חיים בפרודקשן (מיגרציה 0030 הוחלה+אומתה 2026-07-21)** | 2026-07-21 | נובע מחשש שהעלה Liran ("Supabase מול Google — מי בטוח יותר?"). **מועצת-LLM = להישאר על Supabase** (פריצות הן ברמת-אפליקציה, לא ספק-תשתית; Firebase מוסיף מודל-בידוד שני + SDK-דפדפן = יותר משטח-תקיפה; המיגרציה עצמה היא הסיכון). **ביקורת red-team (4 סוכנים, ברמת-קוד):** תכשיט-הכתר — **בידוד בין-דיירים 🟢 איתן (0 דליפה)**; 9 ממצאים, **אף אחד לא בין-דייר**: 1 גבוה (#1 מנהל→בעלים דרך כתיבה-ישירה ל-PostgREST; שורש = הגבלות-תפקיד רק בשכבת-השירות בעוד הטבלאות פתוחות לכתיבה ל-`authenticated`) + בינוניים/נמוכים. **🟢 R2 (שכבת-אפליקציה, [PR #104](https://github.com/Liran-Raz/AVI.APP1/pull/104), main `9745aa3`, חי):** #3 open-redirect ב-/login (`sanitizeNextPath` + הקשחתה לחסום גם `\`, חולץ ל-`lib/safe-path.ts`) + #6 הזרקת-נוסחאות-CSV (`toCsv` מקדים `'` לתאי-טקסט, עם שמירה על סכומים-שליליים). **🟢🟢 R1 (הקשחת-DB, מיגרציה `0029`, [PR #105](https://github.com/Liran-Raz/AVI.APP1/pull/105), main `a342083`; הוחלה+אומתה בפרודקשן ע"י Liran):** טריגרי-שמירה `SECURITY INVOKER` עם שער `current_user='postgres'` המשקפים את `team.service` 1:1 (**אפס שינוי-קוד**) + `revoke insert/delete` על חברויות + `delete` על clients + פיצול-מדיניות client_contacts + 12 החלפות עוזר-מיושן; postflight 3/4/0; **+ job-CI קבוע `validate-write-hardening`** שמוכיח התנהגותית על Postgres אמיתי שכל תקיפה נחסמת וכתיבות-לגיטימיות עוברות. **🟢🟢 R3 מוזג+חי 2026-07-21 ([PR #106](https://github.com/Liran-Raz/AVI.APP1/pull/106) → main `1097aa6`, smoke ירוק כולל הוכחות-חיות):** #8 fail-closed בפרודקשן · #9 CSP נאכף (פרגמטי; nonce=DEV-030) · .strict() על 46 סכמות · **מיגרציה `0030` org-pin הוחלה+אומתה בפרודקשן (מפעיל Liran, postflight = הגדרת-FK מדויקת)**; האחסון→`0031`. ראה פירוט בסעיף DEV-029. |

| DEV-030 | CSP קשיח מבוסס-nonce (שדרוג-המשך לאכיפת ה-CSP של R3) | אבטחה | P3 | ממתין | 2026-07-21 | הוחלט בסבב R3 של DEV-029 (בחירת Liran: "א' עכשיו + ב' מתוכננת להמשך"). האכיפה הפרגמטית חיה; השדרוג המלא = nonce פר-בקשה שמבטל את `'unsafe-inline'` ב-script-src, כך שסקריפט מושתל לא ירוץ כלל גם אם ה-escaping של React ייעקף. מחיר: נגיעה ב-`proxy.ts` (קו-אדום מתועד) + ביטול רינדור-סטטי (הנחיתה תרונדר פר-ביקור) + סבב-QA מלא על שיווק/זכוכית/ווידג'ט-נגישות/i18n. ראה פירוט. |

| DEV-031 | הקשחת-קצה ב-Cloudflare — הפעלת proxy (WAF/DDoS) + Full (strict) + Bot Fight Mode + Page Shield | אבטחה/תשתית | P2 | **הושלם — חי בפרודקשן** | 2026-07-23 | **קונפיג-בלבד (אפס קוד/מיגרציה/PR).** נבע מרצון Liran להגן מפני נחילי-בוטים + DDoS. הופעל proxy (ענן כתום) על **2 רשומות-ה-Vercel בלבד** (`www` + השורש) — כל רשומות-המייל של Resend (MX/TXT/`links-click-tracking`) נשארו **DNS-only** כדי לא לשבור מיילים. SSL הועלה ל-**Full (strict)**. הודלקו **Bot Fight Mode** + **Continuous Script Monitoring (Page Shield)**. **אומת חי:** `CF-RAY` (קצה תל-אביב, `-TLV`), כל המסלולים תקינים (login 200 · tasks/invoicing/reports 307 · APIs 401 · שורש 308→www), ה-**CSP המחמיר תואם** (סקריפט-זיהוי-הבוטים מוגש same-origin מ-`/cdn-cgi/…` → מותר ע"י `script-src 'self'`), ו-**Liran אימת login + יצירת-משימה** דרך ה-proxy. ראה פירוט. |

| DEV-032 | אחסון-קבצים מוצפן (לקוחות/משימות/ספריית-משרד) + הצפנה היררכית (KMS) | פיתוח (רב-סבב) | P2 | **R1a אומת חי בפרודקשן 🟢🟢 (0031+bucket+RLS חיים; העלאה→הורדה-מפוענחת→אובייקט-גולמי-ג׳יבריש = הצפנה-במנוחה מוכחת) · R1b בבנייה — **פיבוט ארכיטקטוני 2026-07-24 (מועצת-LLM 5+5 פה-אחד): בלי Cloud Run.** מסלול-בייטים = הצפנת-דפדפן (Web Crypto) + קישורים-חתומים ישירות ל-Supabase (בלי service-role, אומת). **מאוחד** (כל הגדלים), **עטיפה-כפולה escrow בתוך הבנייה** (מפתח-שחזור פיזי אצל Liran), DR ב-`europe` multi-region. מפתח-האב GCP-KMS (הוקם, לא נפרס). 🟢🟢🟢 **שער-תאימות Node↔WebCrypto ירוק + מקובע (`6c3b926`, 741 בדיקות).** על ענף `feat/dev-032-attachments-r1` (לא ממוזג). נשאר: מודול-דפדפן + routes + escrow + client + סקירה + ניקוי-QA + הדלקה אחת ב-25MB** | 2026-07-24 | תוכנית מאושרת (`~/.claude/plans/breezy-munching-squid.md`). אחסון-קבצים מוצפן על 3 הקשרים; **הצפנת-מעטפת** (KEK-אב ב-Google Cloud KMS `europe` multi-region [תוקן פעמיים 2026-07-24: AWS→GCP (פיזור-עננים), ואז me-west1→europe בעקבות תפיסת-DR של Liran — מפתח אזור-יחיד לא שורד אובדן-אזור] → משרד → לקוח → DEK פר-קובץ, AES-256-GCM); Supabase = מערכת-אמת; דגל `STORAGE_UI` (כבוי). **החלטות נעולות:** מודל-תיקיות (לקוח 4 / משרד 5, 2 תצוגות-על) · ניתוב task-עם-לקוח→תיק-הלקוח · **גודל 25MB** (R1a=ליבת-Vercel ≤4MB, R1b=מסלול-Cloud-Run מרים ל-25MB) · UI-משימה = **אפשרות א׳** (מקטע בדיאלוג-העריכה) · אימות-כתיבה בלי service-role. **✅ בוצע:** מיגרציה `0031` (attachments+encryption_keys, 6 RPCs, טריגר-הקפאה, org-pins) **אומתה על Postgres 17 מקומי וגם הוחלה+אומתה בפרודקשן 2026-07-24 — postflight מדויק `t\|0\|0\|t\|2\|SELECT,UPDATE\|1\|6\|1\|6`** + `0031_harness.sql`+`0031_negative.sql` (64 בדיקות-התקפה)+job-CI `validate-attachments`; מודולי-הצפנה+backend+UI הושלמו (R1a). **🔜 הבא (פיבוט — מסלול מאוחד בדפדפן, לא Cloud Run):** מודול file-crypto איזומורפי (Web Crypto) → עטיפה-כפולה escrow → routes sign-upload/finalize/sign-download (IV מהשרת, AAD=attachment-id) → תקרת-bucket → rewire של ה-client + פרישת מסלול-Node → סקירה אדוורסרית → PR → ניקוי-QA (מפתחות עטופי-dev-KEK בפרודקשן) → יצירת מפתח-escrow (Liran, בעצמו) → הדלקה אחת. **הקשחות-מועצה:** IV-מהשרת, AAD, תקרת-גודל+MIME ב-bucket, row-mint-אחרון. Runbook מפעיל: `docs/GCP_KMS_RUNBOOK.md`. ראה פירוט + memory `project_avi_app.md`. |
...
- **2026-07-19** — **DEV-027 תיקוני-קוד — 4 סבבים שוגרו ואומתו בפרודקשן** (PR #100–#103). סבב A
  (טפסי-auth ציבוריים: `autoComplete` + שגיאות מוכרזות דרך רכיב `FormError` חדש + `<main>`/skip-link
  בכל 6 עמודי-auth), B (הסתרת מוקאפי-הנחיתה מקוראי-מסך, `aria-hidden`), C (ניגודיות `--ink-faint`
  `#67718a`→`#5e6880`, אושר במוקאפ before/after), D (8 טפסים פנימיים — אותו דפוס כמו A, בוצע ב-4
  סוכנים מקבילים + אימות-מרכזי). כל סבב: tsc/lint/560-בדיקות/Vercel/prod-deploy ירוקים; עברית ללא
  שינוי; ללא מיגרציה/deps. DEV-027 עבר מ"תיקוני-קוד דחויים" ל"הושלם — הצהרה + A/B/C/D חיים";
  נותרו מינוריים בלבד (🟡 הסתרת-SVG עקבית, `lang` פר-קטע).
- **2026-07-21** — **DEV-029 ביקורת-אבטחה אדוורסרית 2 + הקשחת-כתיבה — R1+R2 חיים בפרודקשן.** נובע
  מחשש שהעלה Liran על Supabase מול Google; מועצת-LLM הכריעה להישאר על Supabase. ביקורת red-team
  (4 סוכנים) אישרה **בידוד בין-דיירים איתן (0 דליפה)** ומצאה 9 ממצאים (1 גבוה #1 מנהל→בעלים דרך
  כתיבה-ישירה ל-PostgREST + בינוניים/נמוכים), אף לא בין-דייר. **R2** ([PR #104](https://github.com/Liran-Raz/AVI.APP1/pull/104), `9745aa3`) = תיקוני
  שכבת-אפליקציה (#3 open-redirect ב-/login + #6 הזרקת-CSV). **R1** ([PR #105](https://github.com/Liran-Raz/AVI.APP1/pull/105), `a342083`) = מיגרציה
  `0029` הקשחת גבול-הכתיבה ב-DB (טריגרי-שמירה `SECURITY INVOKER` המשקפים את team.service, אפס
  שינוי-קוד) — **הוחלה+אומתה בפרודקשן ע"י Liran** (postflight 3/4/0) + job-CI חדש `validate-write-hardening`
  שמוכיח התנהגותית שכל תקיפה נחסמת. סקירה אדוורסרית-חוזרת של המיגרציה תפסה+תיקנה עקיפת-#4 (re-arm
  הזמנה) + שארית-מחיקת-לקוח לפני ההחלה. 7/9 סגורים חיים; נותר R3 (#8 rate-limiter · #9 CSP · מידע).

---

### DEV-031 — הקשחת-קצה ב-Cloudflare (proxy + Bot Fight Mode + Page Shield)

**רקע:** לאורך שיחה ארוכה על אבטחת-תשתית (שנבעה מחשש-הספק המקורי של Liran) התלבטנו אם
Cloudflare קונה לנו אבטחה. **המסקנות שהוסכמו:** (1) החלפת-מארח (Vercel→Cloudflare/גוגל) **לא**
קונה אבטחה — התקרה (השרת-החי) זהה בכל מארח, וה-DDoS/edge כבר קיים ב-Vercel; (2) ה-proxy **כן**
נותן שכבת-WAF/DDoS/בוטים בחזית, במחיר של צד-תשתית-נאמן-שני שרואה תעבורה **במעבר** (כמו ש-Vercel
כבר רואה 100% ממילא); (3) **ההגנה שמשנה את המשחק על הנתונים היא ההצפנה פר-משרד/פר-לקוח במנוחה**
(פיצ'ר-האחסון), לא ה-proxy — ה-proxy הוא הגנה-לעומק בדלת-הקדמית בלבד, לא "שומר על הדאטהבייס"
(הדפדפן לא מדבר עם ה-DB; ה-proxy אפילו לא על אותו נתיב). Liran בחר להפעיל את ה-proxy כביטוח-זול-והפיך.

**מה הופעל (Cloudflare Dashboard, ע"י Liran; קונפיג-בלבד):**
- **SSL/TLS = Full (strict)** (מ-Automatic/Full). ‏Vercel נותנת תעודה תקפה → strict מאמת נקי, אפס
  סיכון-שבירה. השארנו את תיבות-הציות (**Post-quantum כפוי** / **FIPS**) **ריקות** — שתיהן מגבילות
  בלי תועלת (PQ כבר בברירת-מחדל כשנתמך; FIPS = תקן-ציות אמריקאי שלא חל עלינו, ועלול לכפות קריפטו-ישן).
- **‏DNS proxy (ענן כתום)** על **2 רשומות בלבד** — `www.aviapp1.com` + `aviapp1.com` (שתיהן CNAME
  ל-`…vercel-dns-017.com`). **נשארו אפורות (DNS-only):** `links-click-tracking` (Resend, CNAME —
  המלכודת), `send` MX, `_dmarc` TXT, `resend._domainkey` TXT, `send` TXT. **ה-DDoS-mitigation
  נהיה אוטומטי+תמיד-דלוק** ברגע שהופעל ה-proxy (חינם).
- **Bot Fight Mode** (Security→Bots; JS Detections: On) — נגד נחילי-בוטים.
- **Continuous Script Monitoring / Page Shield** (passive, ניטור-בלבד; ההתראות + PCI-DSS נעולים
  מאחורי שדרוג-בתשלום — לנו אין צורך ב-PCI, לא מעבדים כרטיסי-אשראי).

**אימות (Claude, מהצד הלא-מחובר + Liran על המחובר):** `Server: cloudflare` + `CF-RAY …-TLV`
(מוגש מקצה תל-אביב 🇮🇱; מאחורי הקלעים דפדפן→Cloudflare[TLV]→Vercel[fra1]→Supabase); login 200 ·
tasks/invoicing/reports 307 · APIs 401 · שורש 308→www — בלי לולאה, בלי 526/5xx. **ה-CSP המחמיר
עבר שלם דרך Cloudflare ותואם ל-JS-Detections** (הסקריפט מ-`/cdn-cgi/challenge-platform/scripts/jsd/main.js`
= same-origin → מותר ע"י `script-src 'self'`). **Liran אימת התחברות + יצירת-משימה** (כתיבה ל-DB
עברה את כל השרשרת). Bot Fight Mode לא חוסם מסלולים לגיטימיים.

**הפיך מיידית** (ענן חזרה לאפור = חזרה מלאה). **הערות-המשך:** אם נוסיף ניטור-זמינות חיצוני /
אינטגרציית-שרת-לשרת — ייתכן whitelist ל-Bot Fight Mode. שאר פריטי-ה-Security של Cloudflare
(חסימת-סורקי-AI, AI Labyrinth, security.txt, Scan) — אופציונליים/לא-רלוונטיים לאפליקציה מאחורי-התחברות,
דולגו. **הלקח שחזר בשיחה:** "פיצ'ר-אבטחה ≠ יותר אבטחה בשבילנו" — כל כפתור עובר 3 שאלות (איזה איום /
האם חשופים / מה המחיר); Full(strict) עבר, PQ-כפוי/FIPS נכשלו.

---

### DEV-030 — CSP קשיח מבוסס-nonce (המשך-עתידי לאכיפת ה-CSP)

**רקע:** בסבב R3 של DEV-029 עברה כותרת ה-CSP מ-Report-Only ל**אכיפה פרגמטית** (ראה שם).
ההגנה שנשארה בחוץ במתכוון: חסימת הרצה של סקריפט-inline מושתל — הפוליסי הנאכף כולל
`'unsafe-inline'` ב-script-src כי Next מזריק סקריפטי-hydration inline שאי-אפשר לנעול בלי
nonce פר-בקשה. הסיכון השיורי תאורטי כיום (אפס `dangerouslySetInnerHTML` בקוד; React
עושה escape לכל ערך; ביקורת ה-red-team מצאה 0 XSS) — הוא נהיה רלוונטי רק אם באג עתידי
יעקוף את ה-escaping המובנה.

**מה השדרוג:** יצירת nonce אקראי פר-בקשה + החתמתו על כל `<script>` לגיטימי (Next קורא
אותו מה-header), ואז `script-src 'nonce-…' 'strict-dynamic'` במקום `'unsafe-inline'` —
סקריפט מושתל לא ירוץ כלל. זו "מילת-הסוף" בהגנת XSS.

**המחיר (הסיבה שנדחה):** (1) ה-nonce חייב להיווצר ב-`proxy.ts` — הקובץ הרגיש שנשמר
קפוא למיגרציית-העתיד; (2) כופה רינדור דינמי על כל עמוד — מבטל את הסטטיות של הנחיתה
(TTFB + compute); (3) סבב-QA מלא על כל המשטחים (שיווק/זכוכית/ווידג'ט-נגישות/i18n).

**טריגר מומלץ לביצוע:** לפני סקייל להרבה משרדים / אם ייכנס לקוד משטח HTML גולמי
(`dangerouslySetInnerHTML`) / דרישת-אבטחה חיצונית (ביקורת-לקוח, SOC2).
**נקודת-פתיחה:** האכיפה הקיימת ב-`web/next.config.ts` היא בסיס נקי — השדרוג מחליף רק
את script-src (הכול additive, שום דבר לא נזרק).

---

### DEV-029 — ביקורת-אבטחה אדוורסרית 2 + הקשחת גבול-הכתיבה ב-DB

**נובע מ:** שאלה של Liran — "Supabase היא סטארט-אפ; Google היא ענקית — האם לא בטוח יותר לעבור
אליה מבחינת אבטחה?" ביקשתי קודם **מועצת-LLM** (5 יועצים + peer-review) על האמונה הזו, ואז
**ביקורת-אבטחה+הרשאות מקיפה כולל סייבר-התקפי** בעברית פשוטה.

**מסקנת המועצה (פה-אחד): להישאר על Supabase.** ההיגיון: כמעט כל פריצות-ה-SaaS הן ברמת-אפליקציה
(הרשאות שגויות, סודות שדלפו, פישינג) — **לא** ברמת ספק-התשתית. Supabase = Postgres מנוהל על AWS
(SOC 2). מעבר ל-Firebase/Firestore היה **מגדיל** את משטח-התקיפה (מודל-בידוד שני מבוסס Security
Rules שנכתבים ביד + SDK-דפדפן שחושף את ה-DB), ו**המיגרציה עצמה** (שכתוב-אימות, העברת-נתונים) היא
הסיכון הגדול ביותר. נקודות-עיוורון שהמועצה תיקנה בעצמה: "מעבר לגוגל" ≠ Firestore (Cloud SQL הוא
Postgres מנוהל, מעבר קל יותר); הדאגה הישראלית האמיתית היא **תושבוּת-נתונים/אזור-אחסון** + גיבויים,
לא גודל-הספק.

**הביקורת (4 סוכני red-team, ברמת-קוד):**
- 🟢 **תכשיט-הכתר = איתן.** בידוד בין-דיירים ללא-דליפה: RLS מבוסס-חברוּת + סינון-`org_id` ברפוזיטורי
  + הזרקת-`org_id` מה-session בשירות + RPCs fail-closed + **אין service-role key**. לא נמצאו
  XSS/SQLi/דליפת-סודות/עקיפת-התחברות/דליפה בין-משרדים.
- **9 ממצאים, אף אחד לא בין-דייר.** שורש משותף לרובם: הגבלות-תפקיד ("מי מותר לו") נאכפו **רק
  בשכבת-השירות (Next)**, בעוד שהטבלאות שמרו RLS מסוג `for all` + הרשאות-כתיבה ישירות ל-`authenticated`
  (מפתח-ה-anon נמצא בדפדפן → משתמש-טכני עוקף את האפליקציה וכותב ישירות דרך PostgREST).
  - 🔴 **#1 גבוה** — מנהל (admin) יכול להעלות את עצמו/עובד ל-owner (או להשבית/למחוק חברות) בכתיבה
    ישירה ל-`organization_memberships`. בתוך-המשרד בלבד, דורש admin.
  - 🟠 בינוניים/נמוכים (אותה משפחה): #4 admin מנפיק הזמנת-admin · #5 עובד מוחק איש-קשר בהארד-דליט ·
    #7 עובד מארכב לקוח · #2 חבר-שהושבת עדיין קורא נתוני-הנהלת-חשבונות (עוזרים מיושנים `user_org_id()`/
    `is_admin_or_owner()` מתעלמים מ-`is_active`) · #3 open-redirect ב-/login · #6 הזרקת-נוסחאות-CSV ·
    #8 rate-limiter fail-open · #9 CSP ב-Report-Only.

**הפתרון (בסבבים לפי עדיפות — אישור Liran "לתקן הכל בסבבים"):** להוסיף את אותה הגנה גם ב-DB, בדיוק
כפי שכבר נעשה נכון לצ'אט (מיגרציות 0024/0025). **אף ממצא אינו "בגלל Supabase"** — זהה על כל backend.

**🟢 R2 — שכבת-אפליקציה** ([PR #104](https://github.com/Liran-Raz/AVI.APP1/pull/104), main `9745aa3`, חי):
- **#3** — `sanitizeNextPath(searchParams.get("redirect"), "/tasks")` בדף-הכניסה (הפונקציה כבר קיימת
  ומיושמת ב-OAuth/MFA — נשכחה רק בדף-הכניסה). **הקשחת הפונקציה** לחסום גם `\` + תווי-בקרה (סוגר גם
  את נקודת-ה-mfa); חולצה ל-`web/src/lib/safe-path.ts` (מקור-אמת יחיד, `server/auth/redirect.ts`
  נעשה re-export; נוסף גם ל-native-bridge).
- **#6** — `reports.service.toCsv` מקדים `'` לתאי-טקסט שמתחילים ב-`=+-@`/tab/CR, **עם שומר-מספרים**
  ששומר סכומים-שליליים מפורמטים ("-12.34") אמיתיים (בדיקה-עצמית תפסה רגרסיה שבה הם הפכו לטקסט).
- +7 בדיקות-רגרסיה, 567 סה"כ.

**🟢🟢 R1 — הקשחת גבול-הכתיבה ב-DB · מיגרציה `0029`** ([PR #105](https://github.com/Liran-Raz/AVI.APP1/pull/105), main `a342083`; **הוחלה+אומתה
בפרודקשן ע"י Liran 2026-07-21**, postflight PASS: 3 טריגרים / חברויות SELECT+UPDATE-בלבד / clients
ללא-DELETE / client_contacts 4-מדיניות / 0 עוזרים-מיושנים):
- **המנגנון (תקדים מוכח ב-0027 `enforce_document_immutability`):** טריגרים **`SECURITY INVOKER`** עם
  שער `if current_user='postgres' then return; end if;` בראש → ה-RPCs המהימנים (`bootstrap_org`/
  `accept_invitation`, רצים כ-postgres) + המפעיל עוברים בשקט, והאכיפה נוגעת **רק בכתיבות-`authenticated`
  ישירות**. **אומת שהאפליקציה לעולם לא מבצעת INSERT/DELETE של חברויות כ-authenticated ועושה בדיוק
  UPDATE של אחד מ-{role, is_active, dashboard_access}** → הטריגר משקף 1:1 את `team.service` → **אפס
  שינוי-קוד**.
- **תוכן:** `guard_membership_write` (self-promote/promote-to-owner/admin-נוגע-בבעלים/בעלים-אחרון/
  admin-מציב-employee-בלבד — כולם נחסמים) + `revoke insert,delete` על חברויות · `guard_invitation_role`
  (#4 — `role='admin' AND status='pending'` דורש owner, בודק את **מצב-התוצאה** כדי לסגור עקיפת-re-arm)
  · `guard_client_active` + `revoke delete` על clients (#7 + שארית-הארד-דליט) · פיצול מדיניות `for all`
  של client_contacts (SELECT/INSERT/UPDATE חבר, DELETE admin/owner) (#5) · החלפת 12 מדיניות invoicing+
  bug_reports מהעוזרים-המיושנים (#2).
- **2 סבבי סקירה-אדוורסרית של המיגרציה לפני ההחלה** תפסו+תיקנו עקיפת-#4 אמיתית (re-arm של הזמנת-admin
  מקובלת בלי לשנות role) + שארית-הארד-דליט של לקוח.
- **🆕 הוכחת-סגירה קבועה ב-CI: job `validate-write-hardening`** (`supabase/validation/0029_harness.sql`
  + `0029_negative.sql`) מחיל את 0029 על Postgres חד-פעמי + **מדמה תקיפות דרך PostgREST ישיר**
  (`set role authenticated` + `auth.uid()` GUC) ומוכיח שכל תקיפה **נחסמת** + כתיבות-owner/חבר לגיטימיות
  **עוברות**. ירוק ב-CI. (לקחי-harness: צריך role `service_role` + `grant usage on schema auth` ל-authenticated
  [אחרת auth.uid() נכשל 42501 ונותן "הצלחות" מזויפות]; psql לא מציב `:vars` בתוך `$$…$$` [ליטרלים];
  DELETE חסום-RLS **מסנן בשקט** — לבדוק `row_count`, לא exception.)

**🟢 R3 — תפעול + מידע (מוזג+חי: [PR #106](https://github.com/Liran-Raz/AVI.APP1/pull/106) → main `1097aa6` ‏2026-07-21, במילת-האישור של Liran "תבדוק ותמזג"; פריסת-פרודקשן אומתה חיה — כותרת-ה-CSP הנאכפת מוגשת בווריאנט-הפרודקשן [בלי unsafe-eval/vercel.live], מפתח-זר-ב-body ⇒ ‏400 ‏"Unrecognized key", ‏signin ⇒ ‏401 נקי [לא 503 ⇒ ‏Upstash מוגדר ופעיל]; **מיגרציה `0030` הוחלה+אומתה בפרודקשן באותו ערב** — מפעיל Liran ב-SQL Editor, ‏postflight החזיר את הגדרת-ה-FK המדויקת `FOREIGN KEY (handling_user_id, org_id) REFERENCES organization_memberships(user_id, org_id)`, ‏smoke אחרי-החלה ירוק):**
- **#8 — rate-limiter fail-closed:** production ללא `UPSTASH_*` ⇒ `RateLimitConfigError` טיפוסי (503,
  הודעה גנרית ללקוח; הפירוט בלוג-שרת בלבד) על כל endpoint מוגן — תבנית ה-fail-loud של המייל (F7).
  ‏Preview נשאר fail-open במכוון (אין שם Upstash env); תקלת-Redis חולפת נשארת fail-open (זמינות >
  הקשחה רגעית — הוחלט ותועד בקוד). `VERCEL_ENV` נוסף לסכמת-ה-env. ‏11 בדיקות (resetModules
  פר-תרחיש; ‏assertions על מאפייני-שגיאה, לא `instanceof` — מלכודת-resetModules).
- **#9 — CSP נאכף (בחירת Liran: אכיפה פרגמטית עכשיו, nonce = ‏DEV-030 עתידי):** ‏`Content-Security-Policy`
  אמיתי במקום Report-Only — ‏`connect-src` (חסימת exfiltration ליעד זר), ‏`frame-ancestors`,
  ‏`form-action`, ‏`object-src 'none'`, ‏`base-uri`, ‏`upgrade-insecure-requests`; ‏script/style שומרים
  `'unsafe-inline'` (הזרקות-hydration של Next). ענף-dev מוסיף `unsafe-eval`+`ws:` ‏(HMR); ענף-Preview
  מוסיף vercel.live (טולבר) — אף אחד לא מגיע לפרודקשן. **אומת חי בדפדפן:** כל העמודים הציבוריים +
  שליחת-טופס אמיתית + מתג-שפה + ווידג'ט-נגישות — ‏0 ‏violations, ‏0 שגיאות-קונסול.
- **מידע — `.strict()` על 46 סכמות-קלט** (39 עליונות + 7 מקוננות): מפתח זר ⇒ ‏400 ‏"Unrecognized key"
  במעטפת הקיימת (הוכח end-to-end על ראוט חי). הוחרגו במכוון עם הערת-קוד: ‏`createTaskSchema` (חוזה
  legacy מוצמד-בבדיקה — ‏strip של `status` ישן; אופציה להפוך ל-400 בסבב נפרד), ‏5 סכמות-query
  שנקראות עם `Object.fromEntries` (פרמטר זר כמו `utm_source` חייב להישמט, לא להפיל), ופרמטרי-נתיב
  (מפתחות קבועים ע"י מבנה-התיקיות). אומת **אפס** אי-התאמות client↔schema (כולל דגל-ה-native של
  OAuth שיושב בסכמה בתוך הראוט). כיסוי: ‏`strict-inputs.test.ts` לכל סכמה שהוקשחה.
- **מידע — org-pin: מיגרציה `0030_clients_handler_org_pin.sql`** (בחירת Liran — מיגרציה קטנה עכשיו):
  ‏FK מרוכב `(handling_user_id, org_id) → organization_memberships(user_id, org_id)` — "גורם מטפל"
  לא יכול להצביע על מי-שאינו-חבר-המשרד גם בכתיבת-PostgREST ישירה. ‏ON DELETE NO ACTION (תקדים 0011 —
  ‏cascades של מחיקת-משרד/פרופיל שלמים), ‏MATCH SIMPLE (השדה נשאר nullable/נקי), ‏preflight שמסרב-לרוץ
  על דאטה-מפר + שומרי postgres/single-apply. **אומתה מקומית על PostgreSQL ‏17 אמיתי — הרצף המלא
  ‏0029+0030, ‏8/8 ‏PASS** (כולל תקיפת-`authenticated` ‏23503 + הוכחת-cascade) + צעדי-CI נוספו ל-job
  ‏`validate-write-hardening` (הרחבת-harness / ‏dirty-preflight / החלה / דחיית-כפל / הוכחת-סגירה).
  **החלה בפרודקשן = ידנית ע"י Liran (חבילת-מפעיל בגוף ה-PR).**
- שער: ‏tsc ‏0 · lint ‏0 · **622 בדיקות** (+55) · ‏probes ירוקים · ‏build = ‏Vercel CI (הכלל הקבוע).

**סטטוס: ‏DEV-029 הושלם — הביקורת מוצתה במלואה.** ‏**9/9 ממצאים + שני פריטי-המידע סגורים+חיים
בפרודקשן** (R1: ‏#1/#2/#4/#5/#7 · R2: ‏#3/#6 · R3: ‏#8/#9 + ‏.strict() + ‏org-pin `0030`
שהוחלה+אומתה 2026-07-21). המשך-עתידי
יחיד: ‏DEV-030 ‏(nonce-CSP, ‏P3). תיאום-מספרים: אבטחה לקחה `0029` + `0030`; פיצ'ר-האחסון הנדחה זז
ל-`0031`. **לא נמצאה מעולם דליפה בין-דיירים.**
המלצת-מועצה שנדחתה לעתיד: בדיקת-עמוד-אחד של אזור-אחסון/תושבוּת-נתונים + גיבויים/PITR/שחזור-בדיקה +
תעודות SOC 2/ISO של Supabase.
...
### DEV-027 — נגישות (ת״י 5568 / WCAG 2.0 AA): הצהרה + תיקוני-קוד

**רקע.** ביקורת-נגישות של האתר (2026-07-18) מצאה בסיס טוב אך פערים אמיתיים. שוגר בשני חלקים:
(1) **ההצהרה** (עמוד `/accessibility` he/en, PR #97; נוסח-מחדש משפטי PR #99 — ראה שורת-הטבלה);
(2) **תיקוני-הקוד בפועל** — ההגנה המשפטית החזקה ביותר (הצהרה+ווידג'ט הם משלימים, לא תחליף).
התיקונים תוכננו כ-4 סבבים (ציבורי-קודם, לא-ויזואלי-קודם) ו**כולם שוגרו ואומתו בפרודקשן 2026-07-19**.

**רכיב משותף — `components/ui/form-error.tsx`:** `FormError({id, message, className})` מרנדר
`<p id role="alert">` כשיש message (אחרת null). `role="alert"` = קורא-מסך מכריז את השגיאה ברגע
שהיא נכנסת ל-DOM; ה-`id` מאפשר לשדות להצביע אליו ב-`aria-describedby`. נבנה בסבב A, שימוש-חוזר ב-D.

- **🟢 סבב A — טפסי-auth ציבוריים** ([PR #100](https://github.com/Liran-Raz/AVI.APP1/pull/100), main `ba49083`).
  login/signup/forgot-password/invite-signup: `autoComplete` לכל שדה + שגיאות מוכרזות-ומוצמדות-לשדה
  (`FormError` + `aria-invalid` + `aria-describedby`, לצד ה-toast; ניקוי-בהקלדה). reset-password + mfa
  שודרגו (autocomplete + הפיכת הודעות-השגיאה הקיימות למוכרזות). Landmark `<main id="main">` בכל 6
  עמודי-ה-auth (לא היה אף אחד) + skip-link "דילוג לתוכן" ב-3 מעטפות-ה-glass (מפתח חדש `common.skipToContent`).
- **🟢 סבב B — הסתרת מוקאפי-הנחיתה** ([PR #101](https://github.com/Liran-Raz/AVI.APP1/pull/101), main `958a6ee`).
  `aria-hidden="true"` על ה-HeroDemo (`.hero-demo-wrap`) ועל 5 סצנות-המוקאפ (`.screen-stage`) — כדי
  שקורא-מסך יפסיק להקריא נתוני-דמה מומצאים כאילו הם אמיתיים. התוכן האמיתי (`.screen-cap`) נשאר נגיש.
  אין אלמנטים אינטראקטיביים בפנים (span, לא button) → אין מלכודת-פוקוס.
- **🟢 סבב C — ניגודיות** ([PR #102](https://github.com/Liran-Raz/AVI.APP1/pull/102), main `5c94389`).
  `--ink-faint` `#67718a`→`#5e6880` ב-marketing.css. הכשל היחיד: 4.26:1 על רקע-העמוד `#eaf0fb` (<4.5);
  עכשיו 4.87:1, עדיין הדרגה המעומעמת ביותר. שינוי-ויזואלי → אושר במוקאפ before/after (Liran בחר
  "מינימום חוקי" מספקטרום A–D). טוקן יחיד → מתקן footer/cta-sub/כל טקסט-הדמה. override-הווידג'ט
  (globals.css→שחור) וכפתור-האקצנט (לבן-על-`#2563eb`=5.17) לא נגעו. אומת ב-CSS הבנוי בפרוד.
- **🟢 סבב D — טפסים פנימיים** ([PR #103](https://github.com/Liran-Raz/AVI.APP1/pull/103), main `61ade54`).
  8 טפסים (profile/office/client/contact/task/role/onboarding/invite) קיבלו את דפוס-A: `FormError`
  מוכרז + `autoComplete` לפי-שדה (name/email/tel/organization/street-address; שדות-עסק=off) + aria +
  ניקוי-בהקלדה, תוך שמירת ה-toasts. בוצע ב-4 סוכנים מקבילים + אימות-מרכזי (8 id ייחודיים עם aria תואם,
  toasts נשמרו, אפס role=alert כפול). שיקול: aria על השדה-הראשי ולא על email-אופציונלי.

**אימות (כל הסבבים):** tsc 0 · eslint 0 · 560/560 בדיקות · Vercel Preview + prod-deploy ירוקים ·
עברית ללא שינוי ויזואלי (מלבד הכהיית-הניגודיות המכוונת בסבב C) · אפס מיגרציות/deps.

**נותר (מינורי, דחוי):** 🟡 הסתרת-SVG דקורטיבי עקבית · `lang` פר-קטע · אופציונלי ביקורת-נגישות
מקצועית. ההצהרה/privacy/terms — מומלץ מבט יועץ-משפטי (לא-חוסם).
