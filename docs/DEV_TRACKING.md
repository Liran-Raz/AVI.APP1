...
**סטטוס:** עדיין לא מתוכנן ברמת מימוש. יש כמה החלטות מוצר/טכניות שצריך לסגור לפני שמתחילים
לבנות, כי כל אחת משפיעה על העלות והמורכבות:

1. **אילו "לוגים" בדיוק נאסוף?** אפשרויות (לא סותרות זו את זו):
   - שגיאות קונסולה בדפדפן + בקשות רשת שנכשלו בזמן שלפני הלחיצה (Client-side, קל יחסית
     לאסוף ולצרף לדיווח).
   - "שביל פעולות" — אילו מסכים/כפתורים המשתמש עבר בדקות האחרונות לפני שנתקע (דורש
     tracking קליינטי ייעודי, לא קיים היום).
   - לוגים מצד השרת (Vercel/Next.js request logs) — **מורכב יותר**: Vercel לא חושף היסטוריית
     לוגים בקלות דרך API בלי הגדרת Log Drain נפרד; ידרוש עבודת אינטגרציה נוספת.
   - פרטי הקשר בסיסיים (מי המשתמש, איזה ארגון, איזה דפדפן/גרסה, timestamp) — קל, כדאי בכל מקרה.
2. **איפה מאחסנים את הדיווחים?** טבלה חדשה ב-Supabase (`bug_reports` או דומה) — דורשת
   מיגרציה חדשה + RLS מתאים (כתיבה מותרת לכל משתמש מחובר, קריאה לצוות בלבד). זו מיגרציה
   קטנה ומבודדת, לא קשורה למערך ההרשאות (DEV-001/DEV-003).
3. **מי רואה את הדיווחים ואיך?** האם צריך דשבורד/מסך ניהול פנימי בתוך האפליקציה, או
   שמספיק טבלה שנקרא ידנית ב-Supabase Dashboard / התראה (מייל/Slack) בכל דיווח חדש?
4. **עדיפות מול DEV-001/DEV-003** — פיצ'ר עצמאי, לא תלוי בהם, אפשר להתחיל בכל שלב.

**הצעד הבא:** לסגור את 3 ההחלטות למעלה (במיוחד #1 ו-#3, שהן ההשפעה הגדולה ביותר על
היקף הבנייה) לפני שמתחילים לכתוב קוד.
