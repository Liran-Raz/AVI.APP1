...
| DEV-021 | תפריט-צד נפתח בנייד (מגירה) + סרגל תחתון של 4 | UX/פיתוח | P2 | בתהליך | 2026-07-12 | הכרעות Liran: מגירה **נייבי-זכוכית** (כמו הסייד-בר בדסקטופ) · סרגל תחתון = תור משימות·לקוחות·לוח שבועי·הודעות + כפתור "תפריט" · דסקטופ ללא שינוי · מגירה = ניווט מלא + כרטיס משתמש + קוד משרד + התנתקות. עוצב במוקאפ (`.claude/design-preview/menu-preview.html`) ואושר; מומש ב-`app-shell.tsx` בלבד (ללא מיגרציה). ממתין ל-QA + מיזוג. |
