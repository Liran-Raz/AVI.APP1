...
| DEV-026 | מודול הנהלת-חשבונות (חשבוניות/קבלות) + דוחות + **רישום תוכנה ברשות המסים** | פיתוח (רב-סבב) | P2 | **R1–R3 חיים-רדומים · R4 דוחות+מבנה-אחיד ב-PR** | 2026-07-17 | סבב-על **ledger-first** (שלב א': המשרד מפיק לעצמו; שלב ב' עתידי: ספרים ללקוחות). v1: מסמכים **305/320/330/400** · מסמכים ממוחשבים (R6) · חשבוניות-ישראל (R5; סף ₪10K→₪5K ב-2026) · **מבנה אחיד v1.31** (R4) · רישום במערכת המקוונת (R7). **R1** = מיגרציה **`0027`** (ledgers+documents+counters ללא-דילוגים+קפיאון+RPCs, fail-closed) + פרופיל-עסק + הרשאות `invoices.*` (post-0012) + דגל `INVOICING_UI` (כבוי). אל-יעדים: אין הנה"ח-כפולה/מלאי (INI 1013=0). **בולע את DEV-012** (לוגו+ח.פ על ה-ledger). שערי-בעלים: פרטי-עסק, שער-ממשלתי, תעודת-CA, סימולטור, הגשה. ראה פירוט. |
...
**סטטוס-על:** R1+R2+R3 **מוזגו ל-main** (#79 `f96fc86`, #81 `308cf35`, #82 `00fa2f4`); מיגרציה
0027 **הוחלה+אומתה בפרודקשן** (postflight PASS 10/10); **QA מקומי של ליראן עבר** (+3 תיקוני-סבב;
הורדת-מקור PDF אומתה). הקוד חי-אך-רדום (דגל `INVOICING_UI` כבוי ב-Vercel). **R4 (דוחות +
מבנה-אחיד) ב-PR פתוח.**

**R4 (ענף `feat/dev-026-r4-reports`, PR פתוח):** מנוע מבנה-אחיד + עמוד דוחות —
