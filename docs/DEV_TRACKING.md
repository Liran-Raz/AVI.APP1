...
| DEV-019 | שלב 12 — דרישות מפגישת לקוח (סרגל עליון · מספור משימות · מחזור-חיים חדש · גורם מטפל ללקוח) | פיתוח (רב-סבב) | P2 | בתהליך | 2026-07-11 | סבב-על: מיגרציה `0020` (חיה) + 4 סבבים. **A** (סרגל עליון) חי בייצור. **A/B/C** חיים בייצור; **D** (גורם מטפל ללקוח) QA אושר + במיזוג. ראה פירוט. |
...
**סטטוס:** 🔵 **בתהליך.** A/B/C חיים בייצור; 0020 מאומתת; D (גורם מטפל) QA אושר + במיזוג
— סוגר את הדרישות המקוריות של שלב 12 (R1–R7). תוספת קטנה שביקש Liran ב-QA: כפתור החזרה
ממשימה ב"במעקב" חזרה ל"חדשות" (follow-up נפרד, לא במסמך המקורי).
