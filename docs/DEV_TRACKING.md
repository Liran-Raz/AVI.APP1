...
| DEV-018 | באגים בטאב "התראות" (`/settings`) — רגרסיות מ-DEV-009 חלק 2 | באג | P2 | ממתין | 2026-07-11 | (א) **פונקציונלי:** הטוגל "מתאפס" / הבחירה לא נשמרת (חשד: Radix Tabs מפרק את תוכן הטאב במעבר → הטופס נטען מחדש עם ה-prop ההתחלתי הישן; ו/או ה-PATCH נכשל — לאמת). (ב) **ויזואלי (RTL):** העיגול הלבן של הטוגל יוצא מהמסגרת שמאל כשדלוק — טרנספורם ה-RTL ב-`ui/switch.tsx` שגוי. שניהם קטנים, מומלץ לתקן יחד. |
...
**סטטוס:** ממתין. שניהם קטנים; מומלץ לתקן בסבב קצר אחד.
