...
| DEV-007 | חיווי ויזואלי כשמזינים באיפוס את אותה סיסמה נוכחית | UX/באג | P2 | בתהליך | 2026-07-09 | Supabase דוחה סיסמה זהה לנוכחית, אבל עד עכשיו זה הוצג כ-toast חולף באנגלית — המשתמש לא הבין למה זה נחסם. תוקן בקוד: השרת מסמן `details.reason="same_password"` יציב, והטופס מציג חיווי אדום קבוע בעברית ליד השדה ("הסיסמה החדשה חייבת להיות שונה מהסיסמה הנוכחית"). +5 בדיקות. ממתין לאימות ידני בפרודקשן. |
...
**סטטוס:** קוד מוכן; **ממתין לאימות ידני בפרודקשן** (לנסות להזין באיפוס את אותה
סיסמה נוכחית ולוודא שמופיע החיווי האדום בעברית).
