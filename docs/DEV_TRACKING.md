...
| DEV-010 | בינאום מלא (i18n) — 8 שפות | UX/i18n | P2 | **בפיתוח — סבב 1 (he+en) 7 מסכים חיים** | 2026-07-18 | הורחב מ"תרגום טפסים" ל-**i18n מלא**: עברית, אנגלית, רוסית, גרמנית, צרפתית, יפנית, איטלקית, ערבית (he+ar RTL). **סבב 1 = he+en, מסך-מסך ב-PR קטן.** תשתית (קטלוג עצמאי + עוגייה, ללא next-intl/routing) + מתג-שפה עם דגלים + **7 מסכים חיים בפרודקשן** (הגדרות/משימות/לקוחות/צוות+תפקידים/דשבורד/הודעות + השלד) — PRs #85–#91, main `fef0a82`. אנגלית זמינה, עברית ברירת-מחדל (משתמשים קיימים ללא שינוי). הבא: חשבוניות+דוחות → לוח → onboarding → QA סופי. סבב 2 = 6 השפות הנותרות + פונטים. ראה memory `feature_dev010_i18n.md`. |
