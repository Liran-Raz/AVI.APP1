...
| DEV-010 | בינאום מלא (i18n) — 8 שפות | UX/i18n | P2 | **בפיתוח — סבב 1 (he+en) 9 מסכים חיים** | 2026-07-18 | הורחב מ"תרגום טפסים" ל-**i18n מלא**: עברית, אנגלית, רוסית, גרמנית, צרפתית, יפנית, איטלקית, ערבית (he+ar RTL). **סבב 1 = he+en, מסך-מסך ב-PR קטן.** תשתית (קטלוג עצמאי + עוגייה, ללא next-intl/routing) + מתג-שפה עם דגלים + **9 מסכים חיים בפרודקשן** (הגדרות/משימות/לקוחות/צוות+תפקידים/דשבורד/הודעות/**הנהלת-חשבונות/דוחות** + השלד) — PRs #85–[#92](https://github.com/Liran-Raz/AVI.APP1/pull/92), main `1a0ec89`. PR-8 (חשבוניות+דוחות, ‎+283 מפתחות → 753): ה-PDF/CSV/אזהרות-ממשק-פתוח נשארים עברית; `formatAgorot` עם locale אופציונלי (ברירת-מחדל he-IL — שרת בייט-זהה); `nispach1.*` = 27 שמות רשמיים זהי-בייט לטבלת השרת; `business-types.ts` נמחק. QA Liran עבר, מוזג באישורו. אנגלית זמינה, עברית ברירת-מחדל (משתמשים קיימים ללא שינוי). הבא: לוח → onboarding+הזמנות → auth/נחיתה → QA סופי. סבב 2 = 6 השפות הנותרות + פונטים. ראה memory `feature_dev010_i18n.md`. |
