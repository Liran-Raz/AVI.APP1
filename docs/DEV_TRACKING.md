...
- **R2 — ניהול-קבוצות מלא (נבנה · `0025` הוחלה · ב-PR).** יצירה/שם/הוספה/הסרה/עזיבה/מחיקה + בורר-חברים
...
**סטטוס:** ✅ R1 חי בייצור 2026-07-13 (PR #71, main `d6166b9`, `0024` הוחלה+אומתה). 🟡 **R2 נבנה + `0025`
הוחלה+אומתה בפרודקשן — ב-PR, ממתין QA (Liran) + מיזוג.** R3/R4 פתוחים (כל אחד מוקאפ-קודם + סקירה אדוורסרית).
QA משמר-התנהגות + רב-משתמשי = Liran.
