...
| DEV-032 | אחסון-קבצים מוצפן (לקוחות/משימות/ספריית-משרד) + הצפנה היררכית (KMS) | פיתוח (רב-סבב) | P2 | **R1a אומת חי בפרודקשן 🟢🟢 — מיגרציה `0031` הוחלה+אומתה, bucket+RLS חיים, העלאה→הורדה-מפוענחת→אובייקט-גולמי-ג׳יבריש (הצפנה-במנוחה מוכחת). על ענף `feat/dev-032-attachments-r1` (לא ממוזג). נשאר R1b (Cloud Run 25MB)** | 2026-07-24 | תוכנית מאושרת (`~/.claude/plans/breezy-munching-squid.md`). אחסון-קבצים מוצפן על 3 הקשרים; **הצפנת-מעטפת** (KEK-אב ב-AWS KMS תל-אביב → משרד → לקוח → DEK פר-קובץ, AES-256-GCM); Supabase = מערכת-אמת; דגל `STORAGE_UI` (כבוי). **החלטות נעולות:** מודל-תיקיות (לקוח 4 / משרד 5, 2 תצוגות-על) · ניתוב task-עם-לקוח→תיק-הלקוח · **גודל 25MB** (R1a=ליבת-Vercel ≤4MB, R1b=מסלול-Cloud-Run מרים ל-25MB) · UI-משימה = **אפשרות א׳** (מקטע בדיאלוג-העריכה) · אימות-כתיבה בלי service-role. **✅ בוצע:** מיגרציה `0031` (attachments+encryption_keys, 6 RPCs, טריגר-הקפאה, org-pins) **אומתה על Postgres 17 מקומי — postflight מדויק `t\|0\|0\|t\|2\|SELECT,UPDATE\|1\|4\|1\|6`** (לא הוחלה על שום DB אמיתי) + `0031_harness.sql`. **🔜 הבא:** `0031_negative.sql`+CI · מודולי-הצפנה (`server/crypto`+`server/keys`) · שכבות-אפליקציה · R1b. ראה פירוט + memory `project_avi_app.md`. |
