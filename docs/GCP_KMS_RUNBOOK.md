...
הצפנת-הקבצים, במיקום **`europe` (multi-region)**, ברמת-הגנה **HSM**. המפתח יושב
בכוונה בענן *שונה* מהדאטה (Supabase רצה על AWS) — שום פריצת-ספק יחידה לא מניבה
גם את הצופן וגם את המפתח שפותח אותו.

**למה `europe` ולא תל-אביב (החלטת DR, 2026-07-24):** מפתח באזור-יחיד תלוי באותו
אזור — השבתה אזורית משביתה את פיצ'ר-הקבצים, ואובדן-אזור קטסטרופלי היה הופך את כל
הקבצים המוצפנים לבלתי-קריאים לנצח. במיקום `europe` המפתח **מאוחסן ומוגש פיזית
מכמה מרכזי-נתונים באיחוד האירופי בו-זמנית** — שורד גם השבתה וגם הרס של אזור בודד,
באותו מחיר ובאותם ביצועים. הדאטה עצמה ממילא באיחוד האירופי (פרנקפורט); חובת
"ספרים בישראל" מטופלת בגיבוי-הצופן המתוכנן (R3), לא במיקום מפתח-האב.
...
2. **CREATE KEY RING**:
   - Name: `avi-master`
   - Location type: **Multi-region** ← **europe**  ⚠ חשוב — לא אזור-יחיד
     (תלות-אזור = הסיכון שסגרנו) ולא global (‏global לא תומך HSM).
...
4. פתח את המפתח שנוצר והעתק את **שם-המשאב המלא** (Copy resource name).
   הוא נראה כך:
   `projects/<PROJECT_ID>/locations/europe/keyRings/avi-master/cryptoKeys/master-kek`
   **את הערך הזה נשים ב-Vercel בתור `AVI_GCP_KMS_KEY_NAME`.**

**⚠ אם כבר יצרת Ring או מפתח ב-me-west1 לפני העדכון הזה:** Key Ring ריק אי-אפשר
למחוק וזה בסדר — הוא לא עולה כלום; פשוט התעלם ממנו והמשך עם ה-Ring החדש ב-`europe`.
אם כבר נוצר גם **מפתח** ב-me-west1: פתח אותו ← לשונית VERSIONS ← סמן את הגרסה ←
**Destroy** (ההשמדה מתוזמנת לעוד ~30 יום — זה תקין; היא עוצרת את החיוב של ~$1).
אל תשמיד שום דבר ב-Ring של `europe`.
...
- שירות-המדיה של Cloud Run ירוץ **באותו פרויקט** וישתמש ב-**זהות מובנית**
  ...
  `master-kek` בעת הפריסה (שלב נפרד עם runbook משלו). **השירות עצמו** ירוץ
  ב-**me-west1 (תל-אביב)** — קרוב למשתמשים; רק **המפתח** יושב ב-`europe`,
  והמרחק זניח (קריאת-KMS אחת למשרד לבקשה, עם cache).
