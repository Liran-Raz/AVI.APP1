...
- **מניפסט קבצים (אילו קבצים שייכים למובייל):** [`docs/MOBILE_FILES.md`](./MOBILE_FILES.md)
- **בקלוג כללי של הפרויקט:** [`docs/DEV_TRACKING.md`](./DEV_TRACKING.md)
- **סטטוס-קוד:** M1 **נחת ב-main** `9b0b041` (בתוך #76, **shipped-inert** — ראה רשומת-התקלה ב-DEV_TRACKING). הענף `feat/dev-025-m1-web` מיותר עכשיו; בחידוש-עבודה לפתוח ענף חדש מ-main.
...
| **M2** | Push נייטיבי (מיגרציה `0027`) | מומלץ מאוד | 🔲 |
...
- [ ] M2: מיגרציה `0027` (`device_tokens`) + `@capacitor/push-notifications` + שליחה מהשרת.
...
| שלב | מה | הערות |
|---|---|---|
| **M2** | Push נייטיבי (APNs/FCM) | מיגרציה `0027` = `device_tokens` (⚠️ **לא 0026** — נתפס ע"י צ׳אט R3/R4); שליחה בנקודות-ההתראה הקיימות (שיוך-משימה, מעבר, הודעה). מחליף את DEV-023 (Web Push). |
